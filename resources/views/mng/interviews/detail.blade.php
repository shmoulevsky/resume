@extends('mng.master')
@section('title', __('interview-detail.Title').': '.$interview->resume->full_name)
@section('content')
<div class="mb-5">{{ Breadcrumbs::render('interviews.detail', $interview->resume->full_name, $interview->id) }}</div>

            <h1 class="text-4xl mb-5"><span class="text-gray-400">{{__('interview-detail.Vacancy')}}</span> &laquo;{{$form->name}}&raquo;</h1>
            ...
            <a href="{{route('interviews.list')}}"  class="inline-block border border-gray-200 hover:border-gray-400 font-bold py-2 px-4 rounded my-3">{{__('interview-detail.To_list')}}</a>
